...
SELECT plan(9);
SELECT has_table('cities');
SELECT col_not_null('cities', 'id');
SELECT col_not_null('cities', 'name');
-- Keys
SELECT col_is_pk('cities', 'id');
...
-- Test 9. verify data in cities table is inserted
SELECT is(
  name,
  :city_name,
  'insert_cities verify insert'
) FROM cities
WHERE
  name = :city_name;
